...
For a fuller local quality pass, run `npm run quality:check`. It runs unit tests, production build, dependency audit, and Rust `cargo check`. Use `npm run benchmark` when changing trace parsing, filtering, decoding, or other performance-sensitive paths. The CI quality workflow runs the core compile/test checks on pull requests and pushes to `main`. More detail is in `docs/testing.md`.
